...
<%
		/* 게시글 토탈 개수 얻기*/
		String totalSql = " select count(*) cnt from faq4 where 1=1 "+search+" ";
...
<%
	...
	String sql = " select unq,name,title,content from faq4 "
			   + " where 1=1 "+search+" "
			   + " order by unq desc "
			   + " limit "+startNo+", "+unit+" ";
...
<%@ include file="../js/leftmenu.js"%>
	...
	function fn_action() {
		location="faqSub4Write.jsp?top=board";
	}
	
	function fn_list() {
		location="faqSub4.jsp?top=board";
	...
	<form name="frm" method="post" action="<%request.getRequestURL();%>">
			...
			<nav class="tab_type1">
				<ul>
					...
					<li><a href="../faq4/faqSub4.jsp?top=board" class="on">활동</a></li>
				</ul>
			</nav>
			...
			<table border="0" cellspacing="1" bgcolor="#cccccc" width="950" height="50" align="center">
				...
				<input type="hidden" name="content" value="<%=content %>">
					...
					<td><%=name %></td>
					<td><a href="faqSub4Detail.jsp?top=board&unq=<%=unq %>"><%=title %></a></td>
				</tr>
				<%
							count--;
						}
		...
		<p style="width:100%; text-align:center;">
			...
			%>
				<a href="faqSub4.jsp?top=board&pageNo=<%=i %>"><%=i %></a>
			<%
				}
			%>
		</p>
