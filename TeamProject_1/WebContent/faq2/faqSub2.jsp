...
<%
		/* 게시글 토탈 개수 얻기*/
		String totalSql = " select count(*) cnt from faq2 where 1=1 "+search+" ";
...
<%
	...
	String sql = " select unq,name,title,content from faq2 "
			   + " where 1=1 "+search+" "
			   + " order by unq desc "
			   + " limit "+startNo+", "+unit+" ";
...
<%@ include file="../js/leftmenu.js"%>
	...
	function fn_action() {
		location="faqSub2Write.jsp?top=board";
	}
	
	function fn_list() {
		location="faqSub2.jsp?top=board";
...
<div id="content2">
	<form name="frm" method="post" action="<%request.getRequestURL();%>">
			...
			<nav class="tab_type1">
				<ul>
					<li><a href="../faq1/faqSub1.jsp?top=board">후원</a></li>
					<li><a href="../faq2/faqSub2.jsp?top=board" class="on">홈페이지</a></li>
			...
			<table border="0" cellspacing="1" bgcolor="#cccccc" width="950" height="50" align="center">
				...
				<input type="hidden" name="content" value="<%=content %>">
					...
					<td><%=name %></td>
					<td><a href="faqSub2Detail.jsp?top=board&unq=<%=unq %>"><%=title %></a></td>
				</tr>
				<%
							count--;
						}
		...
		<p style="width:100%; text-align:center;">
			...
			%>
				<a href="faqSub2.jsp?top=board&pageNo=<%=i %>"><%=i %></a>
			<%
				}
			%>
		</p>
